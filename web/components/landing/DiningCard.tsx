...
export function DiningCard({ hall, onClick }: DiningCardProps) {
  ...
  return (
    <Card
      className="group cursor-pointer hover:border-maize transition-all duration-300 flex flex-col h-full border-border bg-card overflow-hidden"
      onClick={onClick}
    >
      {/* Image Area */}
      ...
      <div className="p-4 text-left flex flex-col h-full">
        ...
        <div className="pt-3 border-t border-border mt-auto">
             <Link href={`/menu/${hall.slug}`} className="text-xs text-umich-blue hover:underline">
                View Full Menu →
             </Link>
        </div>
      </div>
